...
// - Avec clubId : vue club (club_admin) → liste uniquement celles du club.
//   La création est automatiquement scopée au club côté backend (club_admin).
...
export function TombolaManager({ clubId = null, title = 'Tombolas', subtitle = 'Crée et gère les tombolas (tirage auto à la date de fin).' }) {
  ...
  async function draw(id) {
    ...
    setActing(id);
    try { const j = await apiFetch(`/api/v2/tombola/${id}/draw`, { method: 'POST' }); showToast(j.data?.drawn ? 'Gagnant tiré !' : `Pas de tirage (${j.data?.reason || '—'})`); load(); }
    catch (e) { showToast('Erreur : ' + e.message); }
    setActing(null);
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl font-black text-bone-50">{title}</h1>
          ...
          <div key={c.id} className="rounded-2xl border border-white/10 bg-ink-800/40 p-5 flex items-center justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {!clubId && c.clubName && <span className="text-[10px] text-bone-500">· {c.clubName}</span>}
              </div>
