// Vercel Serverless Function - GET /api/games/scratch/result?session_id=xxx
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (session.payment_status !== 'paid') {
      return res.status(400).json({ success: false, error: 'Paiement non confirmé', payment_status: session.payment_status });
    }

    const { user_id, organization_id, club, league } = session.metadata;
    const prize = drawScratchPrize();
    const gameId = `scratch-stripe-${session_id.substring(0, 12)}`;
